...
public final class TfIdfFeatures {
	...
	/**
	 * @param document
	 *            The document to create the features for
	 * @param corpus
	 *            The corpus the given document is a part of
	 */
	...
	/**
	 * A feature vector representation of the given document as part of the given corpus.
	 */
	...
	private Float tfIdf(final String dictionaryTerm) {
		/* TF und DF */
		Integer tf = ((TermIndex) document).getTermFrequencyOf(dictionaryTerm);
		tf = tf == null ? 0 : tf;
		Integer df = corpus.getDocumentFrequencyOf(dictionaryTerm);
		df = df == null ? 0 : df;
		/* IDF */
		float idf = (float) Math.log(corpus.getNumberOfDocuments() / (float) df);
		/* TF-IDF */
		float f = tf * idf;
