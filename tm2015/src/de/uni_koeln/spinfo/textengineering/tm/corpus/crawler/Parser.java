 ...
 */
package de.uni_koeln.spinfo.textengineering.tm.corpus.crawler;
 ...
 */
/**
 * Parses a URL into a document representation, using NekoHTML (http://nekohtml.sourceforge.net/).
 * 
 * @author Fabian Steeg
 */
public final class Parser {
	...
	/**
	 * @param url
	 *            The URL of the page to parse into a document
	 * @return A document instance for the given URL
	 */
	public static WebDocument parse(final String url) {
		/*
		 * Als Parser verwenden wir NekoHTML, einen fehlerkorrigierenden Parser (http://nekohtml.sourceforge.net/) der
		 * auf Xerces aufbaut:
		 */
		...
		try {
			...
			WebDocument doc = new WebDocument(url, builder.toString().trim(), links);
			return doc;
	...
	private static void process(final Node node) {
		/*
		 * Wir ermitteln die Elemente über ihre Namen. Man könnte auch mit instanceof testen ob es etwa ein
		 * HTMLParagraphElement ist, allerdings ist das weniger zuverlässig, da etwa XHTML-Dokumente nicht aus solchen
		 * (HTML...), sondern aus anderen Elementen (in einem anderen Namensraum als HTML-Elemente...) bestehen.
		 */
		...
		/*
		 * Jetzt ist der aktuelle Knoten abgearbeitet, jetzt kommt der rekursive Aufruf für den nächsten Knoten auf der
		 * gleichen Schachtelungsebene (wenn es einen gibt):
		 */
		...
		/*
		 * Nachdem jeder je seinen Nachbarn aufgerufen hat, kommt jetzt das ganze mit den untergeordneten:
		 */
